...
function parseProductText(text) {
  ...
  for (const line of lines) {
    ...
    } else if (lower.startsWith("imagens:") || lower.startsWith("imagem:")) {
      const value = line.substring(line.indexOf(":") + 1).trim();
      images.push(...value.split(",").map((image) => image.trim()).filter(Boolean));
...
export function getProducts() {
  ...
  for (let index = 1; index <= PRODUCT_LIMIT; index += 1) {
    ...
    const galleryImages = [
      imageName,
      ...parsed.images,
      ...Array.from({ length: 7 }, (_, extraIndex) => `produto${index}-${extraIndex + 2}.jpg`),
    ];
    const existingImages = [...new Set(galleryImages)]
      .filter((image) => fileExists(path.join(root, image)))
      .map((image) => withSiteBasePath(`/${image}`));
    ...
    products.push({
      ...
      consumptionImage: parsed.consumptionImage
        ? withSiteBasePath(`/${parsed.consumptionImage}`)
        : "",
